...
export class ViajesConductorPage implements OnInit {
  ...
  async ngOnInit() {
    ...
    if (rutLogueado) {
      ...
      if (this.usuarioLogueado?.tipo_usuario === 'Conductor') {
        this.viaje.patchValue({
          conductor: this.usuarioLogueado.nombre,
          rut: this.usuarioLogueado.rut,
          patente: this.usuarioLogueado.patente_auto,
          color_auto: this.usuarioLogueado.color_auto,
          asientos_disponibles: this.usuarioLogueado.asientos_disponibles,
        });
      }
    }
    
    this.fireUsuarioService.getViajesPorConductor().subscribe(viajes => {
      console.log('Viajes del conductor logueado:', viajes);
    });
  ...
  limpiarCamposEspecificos() {
    this.viaje.patchValue({
      nombre_destino: '', // Limpia el campo de nombre_destino
      latitud: '', // Limpia el campo de latitud
      longitud: '', // Limpia el campo de longitud
      distancia_metros: '', // Limpia el campo de distancia_metros
      costo_viaje: '', // Limpia el campo de costo_viaje
      metodo_pago: 'efectivo', // Resetea a un valor por defecto
      numero_tarjeta: '', // Limpia el campo de número de tarjeta
      duracion_viaje: '', // Limpia el campo de duración
      hora_salida: '', // Limpia el campo de hora_salida
      pasajeros: '' // Limpia el campo de pasajeros
    });
    this.botonModificar = true; // Restablece el estado del botón
    this.viaje.get('id__viaje')?.disable(); // Deshabilita el campo de ID
  }
  async onSubmit() {
    ...
    if (this.viaje.valid) {
      ...
      if (viajeCreado) {
        ...
        this.router.navigate(['home/viajes-conductor']);
        this.limpiarCamposEspecificos();

        this.resetMap();   // Limpia el formulario después de crear el viaje
      } else {
  ...
  cargarViajesDelConductor() {
    this.fireUsuarioService.getViajesPorConductor().subscribe(viajes => {
      this.viajess = viajes;
      console.log('Viajes cargados:', this.viajes); // Para verificar en consola
    });
  }
